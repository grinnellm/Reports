\fishName{} (\scienceName{}) in British Columbia are assessed as `r length(allRegions$major)` major and `r length(allRegions$minor)` minor stock assessment regions (SARs), and data are collected and summarized on this scale (\autoref{tabRegions}, \autoref{figBC}).
...
Note that we refer to 'year' instead of 'herring season' in this report; therefore \thisYr{} refers to the `r thisSeason` \fishName{} season.
