...
\newcommand{\legendGear}{Legend: `Other' represents the reduction (\Sexpr{min(yrRange)} to 1970 only), the food and bait, as well as the special use fishery; `RoeSN' represents the roe seine fishery; and `RoeGN' represents the roe gillnet fishery.}
