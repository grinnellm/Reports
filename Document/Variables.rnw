...
\newcommand{\lateBio}{Note that biological sampling data from 2020 are not yet available for distribution; updated tables will be circulated in September.}
...
We will update data summary reports with biological data for 2020 when available.
